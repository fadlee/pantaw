...
import { ActionsButton, IndicatorDot, SystemsTableColumns } from "./systems-table-columns"
...
export default function SystemsTable() {
	...
	const [upSystemsLength, downSystemsLength, pausedSystemsLength] = useMemo(() => {
		return [
			Object.values(upSystems).length,
			Object.values(downSystems).length,
			Object.values(pausedSystems).length,
		]
...
const AllSystemsTable = memo(
	({
		...
		rows,
		colLength,
	}: { table: TableType<SystemRecord>; rows: Row<SystemRecord>[]; colLength: number }) => {
...
const SystemCard = memo(
	({
		...
		table,
		colLength,
	}: { row: Row<SystemRecord>; table: TableType<SystemRecord>; colLength: number }) => {
		const system = row.original
		const { t } = useLingui()
