			...
			cell: (info) => {
				...
				return (
					<>
						...
						<Link href={linkUrl} className="inset-0 absolute size-full" aria-label={name} />
					</>
				)
			},
...
function sortableHeader(context: HeaderContext<SystemRecord, unknown>) {
	...
	// @ts-expect-error
	const { Icon, hideSort, name }: { Icon: React.ElementType; name: () => string; hideSort: boolean } =
		column.columnDef
	...
	return (
		<div className="flex gap-2 items-center tabular-nums tracking-tight w-full">
			...
			<span className="flex-1 min-w-8 grid bg-muted h-[1em] rounded-sm overflow-hidden">
				<span className={meterClass} style={{ width: `${val}%` }} />
			</span>
		</div>
	)
}
				...
				>
					<div className="flex gap-2 items-center tabular-nums tracking-tight">
						<span className="min-w-8 shrink-0">
							{decimalString(rootDiskPct ?? 0, (rootDiskPct ?? 0) >= 10 ? 1 : 2)}%
						</span>
						<span className="flex-1 min-w-8 flex items-center gap-0.5 px-1 justify-end bg-muted h-[1em] rounded-sm overflow-hidden relative">
							{/* Root disk */}
							<span
								className={cn("absolute inset-0", getMeterClass(rootDiskPct ?? 0))}
								style={{ width: `${rootDiskPct ?? 0}%` }}
							/>
				...
				<div className="grid gap-1">
					<div className="grid gap-0.5">
						...
						<div className="flex gap-2 items-center tabular-nums text-xs">
							<span className="min-w-7">{decimalString(rootDiskPct ?? 0, (rootDiskPct ?? 0) >= 10 ? 1 : 2)}%</span>
							<span className="flex-1 min-w-12 grid bg-muted h-2.5 rounded-sm overflow-hidden">
								<span className={getMeterClass(rootDiskPct ?? 0)} style={{ width: `${rootDiskPct ?? 0}%` }} />
							</span>
						</div>
					</div>
						...
						return (
							<div key={name} className="grid gap-0.5">
								...
								<div className="flex gap-2 items-center tabular-nums text-xs">
									...
									<span className="flex-1 min-w-12 grid bg-muted h-2.5 rounded-sm overflow-hidden">
										<span className={getMeterClass(pct)} style={{ width: `${pct}%` }} />
									</span>
								</div>
							</div>
						)
