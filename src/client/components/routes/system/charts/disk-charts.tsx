...
export function DiskUsageChart({
	systemData,
	extraFsName,
}: { systemData: SystemData; extraFsName?: string }) {
	const { chartData, grid, dataEmpty } = systemData

	let diskSize = chartData.systemStats?.at(-1)?.stats.d ?? Number.NaN
	if (extraFsName) {
		diskSize = chartData.systemStats?.at(-1)?.stats.efs?.[extraFsName]?.d ?? Number.NaN
	}
	// round to nearest GB
	if (diskSize >= 100) {
...
export function DiskUtilizationChart({
	systemData,
	extraFsName,
}: { systemData: SystemData; extraFsName?: string }) {
	const { chartData, grid, dataEmpty, showMax, isLongerChart, maxValues } = systemData
	...
	return (
		<div className="grid xl:grid-cols-2 gap-4">
			{Object.keys(extraFs).map((extraFsName) => {
				let diskSize = systemStats.at(-1)?.stats.efs?.[extraFsName].d ?? Number.NaN
				// round to nearest GB
				if (diskSize >= 100) {
					diskSize = Math.round(diskSize)
				}
