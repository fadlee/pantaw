...
export function useSystemData(id: string) {
	...
	useEffect(() => {
		...
		return subscribeKeys(store, [id], (newSystems) => {
			...
			if (sys) {
				setSystem(sys)
				document.title = `${sys?.name} / Pantaw`
			}
		})
	}, [id, systems.length])
	...
	useEffect(() => {
		...
		// Pantaw: system_details tidak ada, skip
		void system.id
	}, [system.id])

	// Pantaw: realtime 1m tidak tersedia (no WebSocket), skip
	useEffect(() => {
		if (!system.id || chartTime !== "1m") {
			return
		}
		// Fallback ke 1h jika chart time 1m diminta
		$chartTime.set("1h")
	}, [chartTime, system.id])
